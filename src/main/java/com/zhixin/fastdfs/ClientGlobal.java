...
import com.zhixin.fastdfs.commons.IniFileReader;
import com.zhixin.fastdfs.commons.MyException;
...
public class ClientGlobal {
  ...
  public static void init(Map  conf) throws IOException, MyException {
	    String[] szTrackerServers;
	    String[] parts;

	    g_connect_timeout = (int) conf.getOrDefault("connect_timeout", DEFAULT_CONNECT_TIMEOUT);
	    if (g_connect_timeout < 0) {
	      g_connect_timeout = DEFAULT_CONNECT_TIMEOUT;
	    }
	    g_connect_timeout *= 1000; //millisecond

	    g_network_timeout = (int) conf.getOrDefault("network_timeout", DEFAULT_NETWORK_TIMEOUT);
	    if (g_network_timeout < 0) {
	      g_network_timeout = DEFAULT_NETWORK_TIMEOUT;
	    }
	    g_network_timeout *= 1000; //millisecond

	    g_charset = (String) conf.get("charset");
	    if (g_charset == null || g_charset.length() == 0) {
	      g_charset = "ISO8859-1";
	    }

	    szTrackerServers = (String[]) conf.get("tracker_server");
	    if (szTrackerServers == null) {
	      throw new MyException("item \"tracker_server\" in conf not found");
	    }
	    ...
	    g_tracker_http_port = (int) conf.getOrDefault("http.tracker_http_port", 80);
	    g_anti_steal_token = (boolean) conf.getOrDefault("http.anti_steal_token", false);
	    if (g_anti_steal_token) {
	      g_secret_key =  (String) conf.get("http.secret_key");
	    }
	  }
